...
% NOTE - With long traces and large numbers of events, building the list of
% all match candidates can take a long time even at O(N^2). Binning reduces
% this to O(N) when the trace is much longer than the search radius.
...
% Part A: Sort the events into bins.
% Sparse cell arrays holding numeric arrays are initialized with empty
% numeric arrays in unused cells. This is fine.
...
for  truthidx = 1:length(evtruth)
  ...
  bidx = ceil(thistruth.sampstart / thistruth.samprate);

  % FIXME - Handle the zero-or-negative sampstart case.
  if 1 > bidx
    bidx = 1;
  end

  thisbin = [];
  if bidx <= length(truthbins)
    thisbin = truthbins{bidx};
  end

  truthbins{bidx} = [ thisbin truthidx ];
...
for testidx = 1:length(evtest)
  ...
  bidx = ceil(thistest.sampstart / thistest.samprate);
  ...
  thisbin = [];
  if bidx <= length(testbins)
    thisbin = testbins{bidx};
  end

  testbins{bidx} = [ thisbin testidx ];

end


% Part B: Build the list of pairs that are in range of each other.

truthbinmax = length(truthbins);
testbinmax = length(testbins);

candcount = 0;

% Iterate through truth bins.
for bidxtruth = 1:truthbinmax

  truthidxlist = truthbins{bidxtruth};

  % Iterate through truth events in this bin.
  for truthidx = truthidxlist

    thistruth = evtruth(truthidx);

    % Iterate through test bins near the truth bin, if valid.
    for bidxtest = (bidxtruth - huntbins):(bidxtruth + huntbins)

      if (bidxtest >= 1) && (bidxtest <= testbinmax)

        testidxlist = testbins{bidxtest};

        % Iterate through test events in this bin.
        for testidx = testidxlist
...
% For small datasets, run against the non-binned algorithm.

if false && ( length(evtest) <= 50 )

  [ fpold, fnold, tpold ] = wlProc_evalEventsVsTruth(evtruth, evtest, ...
    bandlim, snrlim, comparefunc);

  if (fpold ~= fp) || (fnold ~= fn) || (tpold ~= tp)

    disp(sprintf( ...
      '### (fp/fn/tp) mismatch. Binned: %d/%d/%d  Non-binned: %d/%d/%d', ...
      fp, fn, tp, fpold, fnold, tpold ));

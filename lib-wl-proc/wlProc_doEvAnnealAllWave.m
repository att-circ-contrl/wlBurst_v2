...
%parfor evidx = 1:length(oldevents)
  ...
  %
  % Fit amplitude and frequency parameters by simulated annealing.
  ...
  thisfreq = 0.5 * (thisev.f1 + thisev.f2);

  % Start (secs), duration, a1, a2, rollon, rolloff, f1, f2, p1.
  anparamvec = ...
    [ thisdur thisdur thisamp thisamp thisdur thisdur thisfreq thisfreq pi ];
  ...
  [ thisvalid, thiserror ] = anerrfunc(zeros(size(anparamvec)));
  thisev.auxdata.AnnWave_errstart = thiserror;


  % Anneal.
  % We're implicitly passing in "thisev" via "anerrfunc".
  [ thisvec thislongestprobe thisendtotal ] = ...
    wlProc_doAnneal(anparamvec, anerrfunc, maxprobes, maxtotal);
  ...
  thisev = helper_makeCorrectSlopes(thisev, wavedata);
  ...
  thisev.auxdata.AnnWave_maxtunnel = thislongestprobe;
  thisev.auxdata.AnnWave_total = thisendtotal;
  [ thisvalid, thiserror ] = anerrfunc(thisvec);
  thisev.auxdata.AnnWave_errfinal = thiserror;
